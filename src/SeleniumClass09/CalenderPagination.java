...
public class CalenderPagination {
    ...
    public static void main(String[] args) throws InterruptedException {
        ...
        //Select class using when there is dropdown menu
        Select select=new Select(monthDD);
        select.selectByVisibleText("Jul");//getteing month

        //Now we have date table we can use List for it
        List<WebElement> fromDates=driver.findElements(By.xpath("//table[@class ='ui-datepicker-calendar']/tbody/tr/td"));
        //need to iterate from for loop
        for (WebElement fromDate:fromDates ){
            //Need to get the text which is String
            String dateText=fromDate.getText();
            if (dateText.equals("4")){ //Select date
                fromDate.click();
                break;
            }
        }
